...
export class EngineBackendV2 implements StakeEngine {
    url: string;
    ...
    async registerEndpoint(
        ...
        const result = await this.post('staking/v2/endpoint/register', {
    ...
    async claim(
        ...
        const prep = await this.post(
            `staking/v2/staker/${authority.publicKey.toBase58()}/claim/prepare`,
            {
                fund: true
            }
        );
        ...
        const result = await this.post(
            `staking/v2/staker/${authority.publicKey.toBase58()}/claim`,
            {
    ...
    async stake(
        ...
        const prep = await this.post(
            `staking/v2/staker/${staker.publicKey.toBase58()}/stake/${endpoint.toBase58()}/prepare`,
            {
                fund: true,
        ...
        const result = await this.post(
            `staking/v2/staker/${staker.publicKey.toBase58()}/stake/${endpoint.toBase58()}`,
            {
                userSignature: Buffer.from(userSig).toString('base64'),
                message: prep.message
            }
    ...
    async withdraw(
        ...
        for (const endpoint of endpoints) {
            const prep = await this.post(
                `staking/v2/staker/${staker.publicKey.toBase58()}/withdraw/${endpoint.toBase58()}/prepare`,
                {
                    fund: true
                }
            );
            ...
            const result = await this.post(
                `staking/v2/staker/${staker.publicKey.toBase58()}/withdraw/${endpoint.toBase58()}`,
                {
                    userSignature: Buffer.from(userSig).toString('base64'),
                    message: prep.message
                }
    ...
    async transfer(
        ...
        const prep = await this.post(
            `staking/v2/endpoint/${endpoint.toBase58()}/transfer/prepare`,
            {
                fund: true,
        ...
        const result = await this.post(
            `staking/v2/endpoint/${endpoint.toBase58()}/transfer`,
            {
                userSignature: Buffer.from(userSig).toString('base64'),
                message: prep.message
            }
    ...
    async changeBeneficiaries(
        ...
        const prep = await this.post(
            `staking/v2/endpoint/${endpoint.toBase58()}/change-beneficiaries/prepare`,
            {
                fund: true,
        ...
        const result = await this.post(
            `staking/v2/endpoint/${endpoint.toBase58()}/change-beneficiaries`,
            {
                userSignature: Buffer.from(userSig).toString('base64'),
                message: prep.message
            }
